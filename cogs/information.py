...
############################################################## Bot Info #############################################################
    ...
    async def ping(self, ctx):
        em = discord.Embed(title = "<a:pingpongparrot:849455355222425630>Pong!",
                           ...
                           colour = self.client.Blue)
        t = await ctx.reply(embed = em, mention_author=False)
...
############################################################## User Info ############################################################
    ...
    async def userinfo(self, ctx, target: Optional[Member]):
        ...
        em = Embed(title=f"User Info - {target}",
                   ...
                   colour=self.client.Blue)
        em.set_thumbnail(url=user_avatar_url(target))
    ...
    async def avatar(self, ctx, target: Optional[Member]):
        ...
        em.set_author(name=f"{target}'s Avatar", icon_url = user_avatar_url(target))

        em.set_image(url=user_avatar_url(target))
...
############################################################# Server Info ###########################################################
    ...
    async def serveremojis(self, ctx, page: int=1):
        ...
        if output_string != "":
            ...
            await ctx.reply(embed=em, mention_author=False)
        else: await ctx.reply(embed = basic_embed(f"", f"There is nothing here.",self.client.Red,""), mention_author=False)
