...
############################################################## Gamble ###############################################################
    ...
    async def card(self, ctx, target: Optional[Member], bet: str="0"):
        ...
        if self.client.cooldown.count(f"{ctx.author.id}card") == 0:
            ...
            await ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You are on a 5 second cooldown.",self.client.Red,""))
            return
    ...
    async def slots(self, ctx, ammount: str="0"):
        ...
        thumbnail_url = "https://cdn.discordapp.com/attachments/857790584337203251/858850357866135582/Saved_Clip_20210625113108.png"
        user = ctx.author
        if ammount == "0":
            await ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> No ammount was specified.",self.client.Red,f"{command_syntax}"))
            return
...
############################################################### Earn ################################################################
    ...
    async def pray(self, ctx, target: Optional[Member]):
        if ctx.author.bot:
            await ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> Invalid user.",self.client.Red,""))
            return
        ...
        if target is not None:
            if users["pray"] != 0:
                await ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 1 hour cooldown.\n<:info:848526617449070633>You must wait till you can pray for another user.",self.client.Red,f"{await calc_DHMS_left(users['pray'])}"))
                return
    ...
    async def editusercurrency(self, ctx, target: Optional[Member], type: str="", edit_float: float=0):
        ...
        if ctx.author.id in self.client.developerid:
            ...
            if None in (target, type) or edit_float == 0:
                await ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> Incorrect args, use the example below.",self.client.Red,f"{command_syntax}"))
                return
    ...
    async def openaccountcurrency(self, ctx, target: Optional[Member]):
        if ctx.author.id in self.client.developerid:
            ...
            if userdata is None:
                ...
                await ctx.reply(embed = await basic_embed(f"", f"<:info:848526617449070633> An account was oppened for {target}",self.client.Blue,""))
                return
            await ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> {target} already has an account",self.client.Red,""))
