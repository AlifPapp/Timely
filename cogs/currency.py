...
class currency(commands.Cog):
    ...
    #tgive <user> <amount>
    @commands.command()
    async def give(self, ctx, target: Optional[Member], ammount):
        await open_account(self, ctx)
        user = ctx.author
        ...
        if users["savings"] < ammount:
            em = discord.Embed(description = f"<:danger:848526668024250408> You don't have that much money",
                                   color = self.client.Red,
                                   timestamp=datetime.utcnow())
            await ctx.reply(embed = em)
            return
...
#####################################################################################################################################
    # teditbal
    @commands.command()
    async def editbal(self, ctx, target: Optional[Member], edit_int: int=0):
        return
