...
from .functions import calc_DHMS_left, basic_embed, open_account, user_died, user_avatar_url
...
########################################################### Informational ###########################################################
    ...
    async def balance(self, ctx, target: Optional[Member]):
        ...
        if target is not None:
            ...
            if targets is None:
                await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} User doesn't exist in my database.",self.client.Red,f"{command_syntax}"), mention_author=False)
                return
    ...
    async def profile(self, ctx, target: Optional[Member]):
        ...
        em = discord.Embed(color = self.client.Yellow,
                           ...
                           timestamp = datetime.utcnow())
        em.set_author(name=f"{user.name}'s profile", icon_url = user_avatar_url(user))
...
############################################################# Periodic ##############################################################
    ...
    async def daily(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            await ctx.reply(embed = basic_embed("Daily rewards", f"{user} claimed $100!",self.client.Yellow,""), mention_author=False)
            ...
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 1 day cooldown.",self.client.Red,f"{cooldown_string} left"), mention_author=False)
    ...
    async def weekly(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            await ctx.reply(embed = basic_embed("Weekly rewards", f"{user} claimed $1000!",self.client.Yellow,""), mention_author=False)
            ...
            cooldown_string = await calc_DHMS_left(cooldown)
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 1 week cooldown.",self.client.Red,f"{cooldown_string} left"), mention_author=False)
    ...
    async def monthly(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            await ctx.reply(embed = basic_embed("Monthly rewards", f"{user} claimed $2000!",self.client.Yellow,""))
            ...
            cooldown_string = await calc_DHMS_left(cooldown)
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 1 month cooldown.",self.client.Red,f"{cooldown_string} left"))
...
############################################################## Gamble ###############################################################
    ...
    async def card(self, ctx, target: Optional[Member], bet: str="0"):
        ...
        if target == None:
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} No user was specified.",self.client.Red,f"{command_syntax}"), mention_author=False)
            return

        if bet == "0":
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} No ammount was specified.",self.client.Red,f"{command_syntax}"), mention_author=False)
            return

        if ctx.author == target:
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You can't gamble with your self.",self.client.Red,f"{command_syntax}"), mention_author=False)
            return
        ...
        if users["savings"] < bet:
            await ctx.reply(embed = basic_embed(f"", f"You don't have that much money",self.client.Yellow,""), mention_author=False)
            return
        
        await ctx.reply(embed = basic_embed(f"", f"{target.mention} Do you wish to gamble with {user.mention}? `[Yes/No]`",self.client.Yellow,"10 secconds to respond"), mention_author=False)
        def check(m):
        ...
        try:
            ...
            await ctx.reply(embed = basic_embed(f"", f"{target} took to long to answer.",self.client.Yellow,""), mention_author=False)
            return

        if response.content not in ("yes","Yes"):
            await ctx.reply(embed = basic_embed(f"", f"gambling session canceled",self.client.Yellow,""), mention_author=False)
            return
        
        await open_account(self, ctx, target)
        targets = cluster.find_one({"id": target.id})
        if int(targets["savings"]) < bet:
            await ctx.reply(embed = basic_embed(f"", f"{target} has insufficient funds",self.client.Yellow,""), mention_author=False)
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}card") == 0:
            ...
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You are on a 5 second cooldown.",self.client.Red,""), mention_author=False)
            return
    ...
    async def slots(self, ctx, ammount: str="0"):
        ...
        users = cluster.find_one({"id": user.id})
        if users is None:
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} User doesn't exist in my database.",self.client.Red,""), mention_author=False)
            return
        ...
        em = basic_embed(f"Slot Machine", f"[<a:slots:859007300819091476>-<a:slots2:859016177224908830>-<a:slots3:859016218655981608>]",self.client.Yellow,"")
        em.set_thumbnail(url=thumbnail_url)
        message = await ctx.reply(embed = em, mention_author=False)

        em = basic_embed(f"Slot Machine", f"[{emoji_1}-<a:slots2:859016177224908830>-<a:slots3:859016218655981608>]",self.client.Yellow,"")
        em.set_thumbnail(url=thumbnail_url)
        await asyncio.sleep(1)
        await message.edit(embed = em, mention_author=False)

        em = basic_embed(f"Slot Machine", f"[{emoji_1}-{emoji_2}-<a:slots3:859016218655981608>]",self.client.Yellow,"")
        em.set_thumbnail(url=thumbnail_url)
        await asyncio.sleep(1)
        await message.edit(embed = em, mention_author=False)
        ...
        em = basic_embed(f"Slot Machine", f"[{emoji_1}-{emoji_2}-{emoji_3}]",self.client.Yellow,"")
...
############################################################### Earn ################################################################
    ...
    async def pray(self, ctx, target: Optional[Member]):
        if ctx.author.bot:
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} Invalid user.",self.client.Red,""), mention_author=False)
            return
        ...
        if target is not None:
            if users["pray"] != 0:
                await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 1 hour cooldown.\n{self.client.Emojis['info']}You must wait till you can pray for another user.",self.client.Red,f"{await calc_DHMS_left(users['pray'])}"), mention_author=False)
                return

        if self.client.cooldown.count(f"{ctx.author.id}pray") == 0:
            self.client.cooldown.append(f"{ctx.author.id}pray")
        else:
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 30 second cooldown.",self.client.Red,""), mention_author=False)
            return
        ...
        if target is not None:
            if ctx.author != target:
                targets = cluster.find_one({"id": target.id})
                if targets is None:
                    await ctx.reply(embed = basic_embed(f"{self.client.Emojis['danger']} User doesn't exist in my database.",self.client.Red,""), mention_author=False)
                ...
                await ctx.reply(embed = basic_embed("", f"God has bestowed upon **{target.name}** an extra **{time_string}**!!",self.client.Yellow,""), mention_author=False)

                cluster.update_one({"id":ctx.author.id},{"$set":{"luck":round(users["luck"]+0.1,1)}})
                cluster.update_one({"id":ctx.author.id},{"$set":{"pray":3600}})
            else: 
                await ctx.reply(embed = basic_embed("", f"God has bestowed upon you an extra **{time_string}**!!",self.client.Yellow,""), mention_author=False)
        else: 
            await ctx.reply(embed = basic_embed("", f"God has bestowed upon you an extra **{time_string}**!!",self.client.Yellow,""), mention_author=False)
    ...
    async def work(self, ctx, job: str=None, worktimes: str="1"):
        if job == None:
            await ctx.reply(embed = basic_embed("", f"Work where?\n-Use the command `worklist` for a list of jobs.",self.client.Yellow,""), mention_author=False)
            return
        
        if job not in ("Ronalds","ronalds","Maccas","maccas"):
            await ctx.reply(embed = basic_embed("", f"Thats not a valid job!\n-Use the command `worklist` for a list of jobs.",self.client.Yellow,""), mention_author=False)
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}work") == 0:
            ...
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You are on a 7 second cooldown.",self.client.Red,""), mention_author=False)
            return
        cluster = self.client.mongodb["Currency"]["Main"]
        await open_account(self, ctx, ctx.author)
        user = ctx.author
        ...
        if lifespan_amt < 300 or lifespan_amt == users["lifespan"]:
            await ctx.reply(embed = basic_embed("", f"{self.client.Emojis['danger']} You will **DIE!** If you work for that long.",self.client.Red,""), mention_author=False)
        else:
            luck = users["luck"]
            min = 10
            max = 20
    ...
    async def steal(self, ctx, target: Optional[Member]):
        ...
        if users["savings"] < 500:
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You need a minimum of $500 to steal from someone.",self.client.Red,""), mention_author=False)
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}steal") == 0:
            ...
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You are on a 5 minute cooldown.",self.client.Red,""), mention_author=False)
            return
    ...
    async def give(self, ctx, target: Optional[Member], ammount: str="0"):
        ...
        if users["savings"] < ammount:
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You don't have that much money.",self.client.Red,""), mention_author=False)
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}give") == 0:
            ...
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} You are on a 7 second cooldown.",self.client.Red,""), mention_author=False)
            return
    ...
    async def editusercurrency(self, ctx, target: Optional[Member], type: str="", edit_float: float=0):
        ...
        if ctx.author.id in self.client.developerid:
            ...
            if None in (target, type) or edit_float == 0:
                await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} Incorrect args, use the example below.",self.client.Red,f"{command_syntax}"), mention_author=False)
                return
            ...
            if type in ("savings","lifespan","luck"):
                try:
                    ...
                    await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} OverflowError.\nTry a smaller number maybe.",self.client.Red,""), mention_author=False)
                    return
                await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['info']} You successfully edited {target}'s {type} to {target_new_amt}",self.client.Blue,""), mention_author=False)
            else: 
                await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} Invalid cluster type.",self.client.Red,""), mention_author=False)
        return
    ...
    async def openaccountcurrency(self, ctx, target: Optional[Member]):
        if ctx.author.id in self.client.developerid:
            ...
            if userdata is None:
                ...
                await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['info']} An account was oppened for {target}",self.client.Blue,""), mention_author=False)
                return
            await ctx.reply(embed = basic_embed(f"", f"{self.client.Emojis['danger']} {target} already has an account",self.client.Red,""), mention_author=False)
...
async def rich_globalrich(self, ctx, type, page):
    ...
    if rich_string == "":
        await ctx.reply(embed = basic_embed(f"", f"There is nothing here.",self.client.Red,""), mention_author=False)
        return
