...
class currency(commands.Cog):
    ...
    async def balance(self, ctx, target: Optional[Member]):
        ...
        if target is not None:
            ...
            if targets is None:
                await ctx.reply("User doesn't exist in my database for currency")
                return
    ...
    async def rich(self, ctx, page: int=1):
        ...
        rich_string = ""
        for i, x in enumerate(list_of_users, 1):
            if "savings" in x:
    ...
    async def pray(self, ctx, target: Optional[Member]):
        if self.client.cooldown.count(f"{ctx.author.id}pray") == 0:
            ...
            await ctx.reply("Your on a 30 second cooldown")
            return
    ...
    async def work(self, ctx, job: str=None, worktimes: str="1"):
        ...
        if self.client.cooldown.count(f"{ctx.author.id}work") == 0:
            ...
            await ctx.reply("Your on a 7 second cooldown")
            return
        await open_account(self, ctx)
        user = ctx.author
        ...
        if lifespan_amt < 300:
            em = discord.Embed(description = "You will **DIE!** If you work for that long.",
            ...
            if final_work_amt > max:
                final_work_amt = max
            final_work_amt = int(final_work_amt * worktimes)
    ...
    async def steal(self, ctx, target: Optional[Member]):
        ...
        if target == None:
            await ctx.reply("Steal from who though?")
            return
        
        users = self.client.currencydata.find_one({"id": str(user.id)})
        if users["savings"] < 1000:
            await ctx.reply("You need a minimum of $1000 to steal from someone")
            return
        ...
        if final_stealing_chance > 60:
            ...
            self.client.currencydata.update_one({"id":str(user.id)},{"$set":{"savings":savings_amt}})
            self.client.currencydata.update_one({"id":str(target.id)},{"$set":{"savings":savings_amt}})
        else:
...
async def open_account(self, ctx):
    ...
    if userdata is None:
        ...
        em = discord.Embed(title = f"The clock is ticking {user.name}!",
                           description = f"Start using various commands to gain more time, you only have **{DefaultTimeDays} days** left!",
