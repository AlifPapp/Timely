...
class currency(commands.Cog):
    ...
    async def balance(self, ctx, target: Optional[Member]):
        ...
        if target is not None:
            ...
            if targets is None:
                ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> User doesn't exist in my database.",self.client.Red,f"{command_syntax}"))
                return
    ...
    async def profile(self, ctx, target: Optional[Member]):
        ...
        if target is not None:
            targets = cluster.find_one({"id": target.id})
            if targets is None:
                ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> User doesn't exist in my database.\n{command_syntax}",self.client.Red,""))
                return
    ...
    async def pray(self, ctx, target: Optional[Member]):
        if self.client.cooldown.count(f"{ctx.author.id}pray") == 0:
            ...
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 30 second cooldown.",self.client.Red,""))
            return
        cluster = self.client.mongodb["Currency"]["Main"]
        await open_account(self, ctx, ctx.author)
        ...
        if target is not None:
            ...
            await basic_embed(self, ctx,"", f"God has bestowed upon **{target.name}** an extra **{time_string}**!!",self.client.Yellow,"")
            
            cluster.update_one({"id":ctx.author.id},{"$set":{"luck":user["luck"]+0.1}})
        else: 
            ctx.reply(embed = await basic_embed("", f"God has bestowed upon you an extra **{time_string}**!!",self.client.Yellow,""))
    ...
    async def daily(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            ctx.reply(embed = await basic_embed("Daily rewards", f"{user} claimed $100!",self.client.Yellow,""))
            ...
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 1 day cooldown.",self.client.Red,f"{cooldown_string} left"))
    ...
    async def weekly(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            ctx.reply(embed = await basic_embed("Weekly rewards", f"{user} claimed $1000!",self.client.Yellow,""))
            ...
            cooldown_string = await calc_DHMS_left(cooldown)
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 1 week cooldown.",self.client.Red,f"{cooldown_string} left"))
    ...
    async def monthly(self, ctx):
        ...
        if cooldown < 1:
            await open_account(self, ctx, ctx.author)
            user = ctx.author
            ctx.reply(embed = await basic_embed("Monthly rewards", f"{user} claimed $2000!",self.client.Yellow,""))
            ...
            cooldown_string = await calc_DHMS_left(cooldown)
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 1 month cooldown.",self.client.Red,f"{cooldown_string} left"))
    ...
    async def work(self, ctx, job: str=None, worktimes: str="1"):
        ...
        if self.client.cooldown.count(f"{ctx.author.id}work") == 0:
            ...
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You are on a 7 second cooldown.",self.client.Red,""))
            return
        cluster = self.client.mongodb["Currency"]["Main"]
        await open_account(self, ctx, ctx.author)
        user = ctx.author
        ...
        if lifespan_amt < 300 or lifespan_amt == users["lifespan"]:
            ctx.reply(embed = await basic_embed("", f"<:danger:848526668024250408> You will **DIE!** If you work for that long.",self.client.Red,""))
        else:
            luck = users["luck"]
            min = 10
            max = 20
    ...
    async def steal(self, ctx, target: Optional[Member]):
        ...
        if target == None: 
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> No user was specified.",self.client.Red,f"{command_syntax}"))
            return
        
        users = cluster.find_one({"id": user.id})
        if users["savings"] < 500:
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You need a minimum of $500 to steal from someone.",self.client.Red,""))
            return
        
        targets = cluster.find_one({"id": target.id})
        if targets is None:
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> User doesn't exist in my database.",self.client.Red,""))
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}steal") == 0:
            ...
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You are on a 5 minute cooldown.",self.client.Red,""))
            return
    ...
    async def give(self, ctx, target: Optional[Member], ammount: str="0"):
        ...
        if ammount == "0":
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> No ammount was specified.",self.client.Red,f"{command_syntax}"))
            return
        ...
        if users["savings"] < ammount:
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You don't have that much money.",self.client.Red,""))
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}give") == 0:
            ...
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You are on a 7 second cooldown.",self.client.Red,""))
            return
    ...
    async def card(self, ctx, target: Optional[Member], bet: int=0):
        ...
        if ctx.author == target:
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You can't gamble with your self.",self.client.Red,f"{command_syntax}"))
            return
        ...
        if users["savings"] < bet:
            ctx.reply(embed = await basic_embed(f"", f"You don't have that much money",self.client.Yellow,""))
            return
        
        ctx.reply(embed = await basic_embed(f"", f"{target.mention} Do you wish to gamble with {user.mention}?\n`Yes/No`",self.client.Yellow,"5 secconds to respond"))
        def check(m):
            return m.author == target and m.content in ("yes","Yes")
        try:
            response = await self.client.wait_for('message', check=check, timeout=5)
        except asyncio.TimeoutError:
            ctx.reply(embed = await basic_embed(f"", f"{target} took to long to answer.",self.client.Yellow,""))
            return

        if response.content not in ("yes","Yes"):
            ctx.reply(embed = await basic_embed(f"", f"gambling session canceled",self.client.Yellow,""))
            return
        
        await open_account(self, ctx, target)
        targets = cluster.find_one({"id": target.id})
        if int(targets["savings"]) < bet:
            ctx.reply(embed = await basic_embed(f"", f"{target} has insufficient funds",self.client.Yellow,""))
            return
        ...
        if self.client.cooldown.count(f"{ctx.author.id}gamble") == 0:
            ...
            ctx.reply(embed = await basic_embed(f"", f"<:danger:848526668024250408> You are on a 10 second cooldown.",self.client.Red,""))
            return
