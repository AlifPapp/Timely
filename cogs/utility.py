    ...
    async def oldest(self, ctx, page: int=1):
        ...
        for x in ctx.guild.members:
            if x.created_at is not None:
                ...
                if cycle_int > int((page*10) - ((page-1)*10)):
                    old_string += f"**{cycle_int}** - {x} - {x.created_at.strftime('%d/%m/%Y')}\n"
                if cycle_int == int(page*10): break
        
        em = discord.Embed(color=self.client.Blue)
        em.add_field(name=f"Oldest accounts in **{ctx.guild.name}**", value=old_string, inline=False)
