...
class other(commands.Cog):
    ...
    async def userinfo(self, ctx, target: Optional[Member]):
        ...
        status_string = ""
        online_status = discord.Status.online, discord.Status.idle, discord.Status.dnd, discord.Status.invisible
        if target.desktop_status in online_status: status_string += "<:computer:851705110408462367>"
        if target.mobile_status in online_status: status_string += "<:mobile:851706779812233227>"
        if target.web_status in online_status: status_string += "<:web:851740865666875423> "
