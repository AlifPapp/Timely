...
class help(commands.Cog):
    ...
    async def currency(self, ctx):
        ...
        em = discord.Embed(title = "Currency commands",
                           description = "These are the commands for a unique economy system based on the movie: \n`In Time`.",
                           colour = self.client.Green,
                           timestamp=datetime.utcnow())

        em.add_field(name="Informational:",value=f"`{p}balance`, `{p}profile`, `{p}rich`, `{p}globalrich`",inline=True)
        em.add_field(name="Periodic:",value=f"`{p}daily`, `{p}weekly`, `{p}monthly`",inline=True)
        em.add_field(name="Earn:",value=f"`{p}pray`, `{p}work`, `{p}steal`, `{p}give`, `{p}card`",inline=True)
    ...
    async def information(self, ctx):
        ...
        em.add_field(name="Bot Info:",value=f"`{p}prefix`, `{p}ping`, `{p}invite`, `{p}creator`, `{p}developers`, `{p}botinfo`",inline=True)
        em.add_field(name="Server Info:",value=f"`{p}serverinfo`, `{p}oldest`, `{p}newest`",inline=True)
        em.add_field(name="User Info:",value=f"`{p}userinfo`, `{p}avatar`",inline=True)
...
#####################################################################################################################################
    ...
    async def card(self, ctx):
        ...
        em.add_field(name="**Values**",value=f"Cooldown: 10 Secconds",inline = False)
