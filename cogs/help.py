...
class help(commands.Cog):
    ...
    async def help(self, ctx):
        await ctx.send(embed = await help_home_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp Currency
    @help.command()
    async def currency(self, ctx):
        await ctx.send(embed = await help_currency_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp Information
    @help.command(aliases=['info','information'])
    async def informational(self, ctx):
        await ctx.send(embed = await help_information_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp Counting
    @help.command()
    async def counting(self, ctx):
        await ctx.send(embed = await help_counting_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp Settings
    @help.command()
    async def settings(self, ctx):
        await ctx.send(embed = await help_settings_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp reddit
    @help.command()
    async def reddit(self, ctx):
        await ctx.send(embed = await help_reddit_embed(self), view = DropdownView(self, ctx, "help"))

    # thelp other
    @help.command()
    async def other(self, ctx):
        await ctx.send(embed = await help_other_embed(self), view = DropdownView(self, ctx, "help"))
...
class DropdownHelp(discord.ui.Select):
    def __init__(self, self2):
        self.self2 = self2
        options = [discord.SelectOption(label='Home', emoji='<:home:896943982251216899>'),
                  discord.SelectOption(label='Currency', emoji='<:coin:862881131799248937>'),
                  discord.SelectOption(label='Counting', emoji='<:count:862882885916033054>'),
                  discord.SelectOption(label='Reddit', emoji='<:reddit_icon:869174075250061343>',),
                  discord.SelectOption(label='Informational', emoji='<:info:862881844265746442>'),
                  discord.SelectOption(label='Settings', emoji='<:static_cog:862507223062151168>'),
                  discord.SelectOption(label='Other', emoji='<:plus:897319598662254662>')]
        super().__init__(placeholder='Browse Commands', min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "Home":
            await interaction.response.edit_message(embed = await help_home_embed(self.self2))
        if self.values[0] == "Currency":
            await interaction.response.edit_message(embed = await help_currency_embed(self.self2))
        if self.values[0] == "Counting":
            await interaction.response.edit_message(embed = await help_counting_embed(self.self2))
        if self.values[0] == "Reddit":
            await interaction.response.edit_message(embed = await help_reddit_embed(self.self2))
        if self.values[0] == "Informational":
            await interaction.response.edit_message(embed = await help_information_embed(self.self2))
        if self.values[0] == "Settings":
            await interaction.response.edit_message(embed = await help_settings_embed(self.self2))
        if self.values[0] == "Other":
            await interaction.response.edit_message(embed = await help_other_embed(self.self2))

class DropdownView(discord.ui.View):
    def __init__(self, self2, ctx, item, *, timeout = 120.0):
        ...
        if item == "help":
            self.add_item(DropdownHelp(self2))

async def help_home_embed(self):
    p = self.client.serverprefix
    ...
    em.add_field(name="<:info:862881844265746442> Informational",value = f"`{p}help information`",inline=True)
    em.add_field(name="<:static_cog:862507223062151168> Settings",value = f"`{p}help settings`",inline=True)
    em.add_field(name="<:plus:897319598662254662> Other",value = f"`{p}help settings`",inline=True)
    return em

async def help_currency_embed(self):
    p = self.client.serverprefix
    ...
    em.set_footer(text=f"{p}help <cmd> to see more details")
    return em

async def help_information_embed(self):
    p = self.client.serverprefix
...
async def help_counting_embed(self):
    p = self.client.serverprefix
...
async def help_settings_embed(self):
    p = self.client.serverprefix
...
async def help_reddit_embed(self):
    p = self.client.serverprefix
