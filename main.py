...
class Bot(commands.Bot):
    ...
    async def on_guild_join(self, guild):
        cluster = self.mongodb["Settings"]["ServerPrefix"]
        ...
        if self.user.id != 836198930873057290: return
        #Send message in TimelyBot Official
        channel = self.get_channel(850241543495352351)
        em = discord.Embed(title = "Joined Server",
                           description = f"**Name:** {guild}\n**ID:** {guild.id}\n**MemberCount:** {guild.member_count}\n**New GuildCount:** {len(self.guilds)}",
                           color = self.Blue,
    ...
    async def on_guild_remove(self, guild):
        ...
        em = discord.Embed(title = "Left Server",
                           description = f"**Name:** {guild}\n**ID:** {guild.id}\n**MemberCount:** {guild.member_count}\n**New Bot's GuildCount:** {len(self.guilds)}",
                           color = self.Red,
                           timestamp=datetime.utcnow())
        await channel.send(embed = em)
client = Bot()
