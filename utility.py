...
class other(commands.Cog):
    ...
    async def oldest(self, ctx, page: int=1):
        await oldest_youngest(self, ctx, page, "Oldest")
    # tyoungest
    @commands.command()
    async def youngest(self, ctx, page: int=1):
        await oldest_youngest(self, ctx, page, "Youngest")

async def oldest_youngest(self, ctx, page, sort_type):
    page = abs(int(page))
    if page > 99: page = 99
    if sort_type == "Oldest": members_sorted = ctx.guild.members[((page-1)*10):(page*10)]
    if sort_type == "Youngest": 
        members_sorted = ctx.guild.members[::-1]
        members_sorted = members_sorted[((page-1)*10):(page*10)]
    old_string = ""
    cycle_int = 0
    for x in members_sorted:
        if x.created_at is not None:
            cycle_int = cycle_int + 1
            old_string += f"**{cycle_int+((page-1)*10)}** - {x} - {x.created_at.strftime('%d/%m/%Y')}\n"
        
    em = discord.Embed(color=self.client.Blue)
    em.add_field(name=f"{sort_type} accounts in **{ctx.guild.name}**", value=old_string, inline=False)
